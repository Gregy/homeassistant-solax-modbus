...
class SolaXModbusSelect(SelectEntity):
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        # Handle autorepeat for selects (same pattern as buttons)
        if self.entity_description.autorepeat:
            duration = self._hub.data.get(self.entity_description.autorepeat, 0)
            autorepeat_set(self._hub.data, self._key, time() + duration - 0.5)
